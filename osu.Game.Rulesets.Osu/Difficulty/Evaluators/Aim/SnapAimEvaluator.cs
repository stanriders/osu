...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators.Aim
{
    public static class SnapAimEvaluator
    {
            ...
            if (prevPrevMovement != null)
            {
                ...
                if (Math.Max(currentMovement.Time, previousMovement.Time) < 1.25 * Math.Min(currentMovement.Time, previousMovement.Time)) // If rhythms are the same.
                {
                    ...
                    acuteAngleBonus *= velocityInfluence *
                                       DiffUtils.Smootherstep(DiffUtils.MillisecondsToBPM(currentMovement.Time, 2), 300, 400) *
                                       DiffUtils.Smootherstep(currentMovement.Distance, 0, diameter * 2);
                }
                ...
                double wiggleBonus = velocityInfluence
                                     * DiffUtils.Smootherstep(currentMovement.Distance, radius, diameter)
                                     * Math.Pow(DiffUtils.ReverseLerp(currentMovement.Distance, diameter * 3, diameter), 1.8)
                                     * DiffUtils.Smootherstep(currAngle, double.DegreesToRadians(110), double.DegreesToRadians(60))
                                     * DiffUtils.Smootherstep(previousMovement.Distance, radius, diameter)
                                     * Math.Pow(DiffUtils.ReverseLerp(previousMovement.Distance, diameter * 3, diameter), 1.8)
        ...
        private static double vectorAngleRepetition(Movement current, Movement previous, Movement prevPrevious)
        {
            ...
            double stackFactor = DiffUtils.Smootherstep(current.Distance, 0, OsuDifficultyHitObject.NORMALISED_DIAMETER);
