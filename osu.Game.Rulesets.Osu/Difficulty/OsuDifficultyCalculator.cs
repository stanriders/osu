...
namespace osu.Game.Rulesets.Osu.Difficulty
{
    public class OsuDifficultyCalculator : DifficultyCalculator
    {
        ...
        protected override DifficultyAttributes CreateDifficultyAttributes(IBeatmap beatmap, Mod[] mods, Skill[] skills, double clockRate)
        {
            ...
            double aimLengthBonus = 1 + Math.Min(1.5, aimRelevantObjectCount / 375.0) +
                                    (aimRelevantObjectCount > 562.5 ? Math.Log10(aimRelevantObjectCount / 562.5) : 0);
            aimRating *= Math.Cbrt(aimLengthBonus);

            double aimNoSlidersLengthBonus = 1 + aimNoSlidersRelevantObjectCount / 375.0;
            aimRatingNoSliders *= Math.Cbrt(aimNoSlidersLengthBonus);

            double speedLengthBonus = 1 + 0.5 * Math.Min(1.0, speedRelevantObjectCount / 500.0) +
